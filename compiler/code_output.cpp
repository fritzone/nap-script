...
void second_part_target_unknown_source_register(nap_compiler* _compiler, variable* dest, int level)
{
    code_stream(_compiler)
            << C_COMMA
            << reg() << get_reg_type(dest->i_type) << C_PAR_OP << level << C_PAR_CL
            << NEWLINE;
}
...
void mov_target_register_source_var(nap_compiler* _compiler, expression_tree* var_node, int reqd_type, int level,
                      const method* the_method, call_context* cc, int forced_mov, bool& psuccess)
{
    mov_start_register(_compiler, reqd_type, level);

    compile(_compiler, var_node, the_method, cc, level, reqd_type, forced_mov, psuccess);
...
void mov_target_register_source_var(nap_compiler* _compiler, call_context* cc, variable* var, int level)
{
    code_stream(_compiler)
            ...
            << reg() << get_reg_type(var->i_type) << C_PAR_OP << level << C_PAR_CL
            << C_COMMA
            << fully_qualified_varname(cc, var) << NEWLINE;
}
...
void operation_target_register_source_register(nap_compiler* _compiler, int req_type_1, int level_1, int req_type_2, int level_2 )
{
    code_stream(_compiler)
            << mov()
            << SPACE
            << reg() << get_reg_type(req_type_1) << C_PAR_OP << level_1 << C_PAR_CL
            << reg() << get_reg_type(req_type_2) << C_PAR_OP << level_2 << C_PAR_CL
            << NEWLINE;
}
...
void call_internal_grow_with_register(nap_compiler* _compiler, call_context* cc, variable* var, int level)
{
    code_stream(_compiler)
            ...
            << C_PAR_OP
                << fully_qualified_varname(cc, var)
                << C_COMMA
                << reg() << int_() << C_PAR_OP << level << C_PAR_CL
            << C_PAR_CL
            << NEWLINE;
}
...
void mov_target_index_register_source_atomic(nap_compiler* _compiler, int& idxc,
                                     ...
                                     call_context* cc, int level, int forced_mov, bool& psuccess )
{
    ...
    compile(_compiler,indxs, the_method, cc, level, int_type, forced_mov, psuccess);
