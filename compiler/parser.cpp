    ...
    while (cur_save < f->content_size && !phrase_read)
    {
        ...
        if (is_phrase_delimiter(f->content[cur_save]))
        {
            ...
            if (f->content[cur_save] == '{')
            {
                ...
                while (is_whitespace(f->content[save_cur_save]))
                {
                    save_cur_save --;
                }
            ...
            if (regular_delim)
            {
                *delim  = f->content[cur_save];
                ...
                else if (C_CLOSE_BLOCK == f->content[cur_save])
                {
                    skipper = 0;
                }
            }
            else
            {
                ...
                while (cur_save < f->content_size && !can_stop)
                {
                    if (C_QUOTE == f->content[cur_save])       /* skip the strings */
                    {
                        ...
                        {
                            if (C_QUOTE == f->content[cur_save] && C_BACKSLASH != f->content[cur_save])
                            {
                                can_stop2 = 1;
                            }
                            cur_save ++;
                    ...
                    if (C_OPEN_BLOCK == f->content[cur_save])
                    {
                        level ++;
                    }
                    else
...
{
    while (is_whitespace(pf->content[pf->position]))
    {
        pf->position++;
    }
}
